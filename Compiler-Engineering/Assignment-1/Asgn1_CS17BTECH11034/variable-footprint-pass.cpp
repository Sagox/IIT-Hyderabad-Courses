...
namespace {
	struct variableFootprint : public FunctionPass  {
		...
		bool globalChecked = false;
		bool arcInfoPrinted = false;
		...
		// This recursive function is used to update the read and write values in the scopeInfo structs for those
		// instructions which are present in the loops, the ir file may not be in ssa so we cannot use scev
		// so I have tried to get the upper bound and step sizes from the loop blocks, this works for onlyvery simple
		// loop where the iv starts from 0 goes to some n in steps of a k.
		void updateRWVals(Loop *L, std::vector<scopeInfo> &scopeInfos, int loopTrip, AAResults *AA) {
			...
			// iterate over all the instructions in the header block
			for(t = bb->begin(); t!= bb->end();t++) {
				// assuming the branch instruction in the header is where the loop can be exited and assuming that
			...
			for(t = bb->begin(); t!= bb->end();t++) {
				// we look for the add opcode in this block since we assume the loop has an incremental iv, we get the
				// operand 1 for the add instruction assuming that it is the step for the loop iv.
				if(t->getOpcodeName() == std::string("add"))
					step = (dyn_cast<ConstantInt>(t->getOperand(1)))->getSExtValue();
			}
			// the value of the previous loop trip is required because the instructions which were counted in the parent loop
			// must be subtracted by the previous number here.
			int loopTripPrev = loopTrip;
			// the loop trip for the current depth would just be the multiplication of the the two looptrips, initialized to 1 by the
			// first call to the function
			loopTrip = loopTrip*bound/step;
			// iterate over all the basic blocks in this loop
		...
		bool runOnFunction(Function &F) {
			...
			if(!arcInfoPrinted) {
				...
				errs() << "LLVM Source Repository: https://github.com/llvm/llvm-project\n";
				errs() << "LLVM Commit Hash: f49937eca01940dde75fda8b4d4fc479a6895f1a\n";
			...
			// if(scopeInfos.size() <= 0)
			// 	return false;
			// errs() << scopeInfos.size() << "\n";
			// print the output in the required pattern	

			for(auto i=0;i<scopeInfos.size();i++) {
				errs() << "Variable Name: " << varName << "\n";
				errs() << "Variable Scope: " <<scopeInfos[i].startLine <<":"<<scopeInfos[i].FinishLine<<"\n"; 
				errs() << "Footprint: ";
				for(auto j=0;j<scopeInfos[i].lines.size();j++) {
					errs() << scopeInfos[i].lines[j];
					if(j!=scopeInfos[i].lines.size()-1)
						errs() << ", ";
				}
				errs() <<"\n";
				errs() << "Number of Reads: " << scopeInfos[i].reads << "\nNumber of Writes: " << scopeInfos[i].writes << "\n";
			}
			return false;
		}
	};
