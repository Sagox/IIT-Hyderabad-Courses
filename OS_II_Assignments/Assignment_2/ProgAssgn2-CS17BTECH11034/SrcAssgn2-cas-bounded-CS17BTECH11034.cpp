...
// waiting thread list required in bounded cas
vector<bool> waiting;
time_t tt;
...
void* testCS(void *param) {
	...
	for (auto i=1;i<=k;i++) {
		...
		// push waiting time into waiting matrix
		waiting_time_matrix[id].push_back(float(duration));
		
		 // recording enter times
...
{
	// log file
	pFile = fopen ("bounded-cas-output.txt","w");
